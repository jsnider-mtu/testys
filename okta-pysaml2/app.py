...
metadata_url_for = {
    ...
    # WARNING WARNING WARNING
    'Okta': 'https://parkmobile.okta.com/app/exkfibh8avlK2nYBH357/sso/saml/metadata'
    }
...
def putEvent(eventTime, eventName):
    client = boto3.client('autoscaling', region_name='us-east-1')
...
def main_page():
    ...
    if request.method == 'POST':
        ...
        if not formEventName:
            ...
            submittedDateTime = ''.join(
                [str(formDate), "T", str(formTime), ":00Z"])
            submittedEventName = formEventName.replace(" ", "-")
            print("submittedDateTime: %s" % submittedDateTime)
            print("submittedEventName: %s" % submittedEventName)
            #putEvent(submittedDateTime, submittedEventName)
    #data = getEvents()
    data = {'ScheduledUpdateGroupActions': [{'ScheduledActionName': 'test-action', 'Schedule': "2022-01-27T15:00:00", 'Timezone': 'UTC'}]}
    return render_template('main_page.html', idp_dict=metadata_url_for, data=data['ScheduledUpdateGroupActions'], submittedDateTime=submittedDateTime, submittedEventName=submittedEventName)
